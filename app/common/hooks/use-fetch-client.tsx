...
import { queryStringToJSON, toQueryString } from "../url-transform";
...
function updateUrlWithFilters({
  ...
  Object.entries(data).forEach(([key, value]) => {
    url.searchParams.set(
      noPrefixQuery || !componentId
        ? camelToSnake(key)
        : `${componentId}-${camelToSnake(key)}`,
      value?.toString() || ""
    );
  });
  // remove empty params
...
function getFiltersFromUrl({ componentId, noPrefixQuery }: IParamsProps): any {
  const url = new URL(window.location.href);
  let filters: any = {};
