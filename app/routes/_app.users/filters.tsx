...
import { UsersFilterModel } from "~/models/users.model";
...
export function Filter() {
  ...
  const form = useForm<UsersFilterModel>({
    resolver: zodResolver(UsersFilterModel.schema),
    defaultValues: new UsersFilterModel(),
  });
  ...
  const mapQueryParamToForm = () => {
    ...
    searchParams.forEach((value, key) => {
      ...
      const formKey = toCamelCase(key) as keyof UsersFilterModel;
  ...
  const onClearFilters = () => {
    form.reset(new UsersFilterModel());
    navigate(``);
  };
